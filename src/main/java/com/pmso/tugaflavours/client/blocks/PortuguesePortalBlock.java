...
public class PortuguesePortalBlock extends Block {
	...
	public static final IntegerProperty LIT = BlockStateProperties.AGE_0_2;

	public PortuguesePortalBlock() {
		super(Block.Properties.create(Material.ROCK).hardnessAndResistance(1.3f, 3.0f).sound(SoundType.GLASS)
				.tickRandomly());
	...
	public void placeInPortal(Entity entity, ServerWorld serverWorld) {
        ...
        for (int i = 1; i < 4; i++) {
            serverWorld.setBlockState(portalBottom.add(2, 0, 2).up(i), ModBlocks.CORK_BLOCK.get().getDefaultState());
            serverWorld.setBlockState(portalBottom.add(2, 0, -2).up(i), ModBlocks.CORK_BLOCK.get().getDefaultState());
            serverWorld.setBlockState(portalBottom.add(-2, 0, 2).up(i), ModBlocks.CORK_BLOCK.get().getDefaultState());
            serverWorld.setBlockState(portalBottom.add(-2, 0, -2).up(i), ModBlocks.CORK_BLOCK.get().getDefaultState());
        }
